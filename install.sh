...
expand_templates() {
    ...
    (
        ...
        # Write token registry (expiry tracking + optional extra PATs).
        # Uses a here-doc rather than envsubst so ${VAR:+...} conditionals are
        # preserved and empty optional vars produce no export line.
        {
            echo "# GitHub token registry — generated by install.sh — do not edit directly."
            echo "# Use gh-as <NAME> <args> or gh-token-check. See bash/.bashrc."
            echo ""
            echo "export GH_TOKEN_SBATES130272='${GH_TOKEN_SBATES130272}'"
            [[ -n "${GH_TOKEN_SBATES130272_EXPIRES:-}" ]] && \
                echo "export GH_TOKEN_SBATES130272_EXPIRES='${GH_TOKEN_SBATES130272_EXPIRES}'"
            echo ""
            echo "export GH_TOKEN_STEBATES_AMDENG='${GH_TOKEN_STEBATES_AMDENG}'"
            [[ -n "${GH_TOKEN_STEBATES_AMDENG_EXPIRES:-}" ]] && \
                echo "export GH_TOKEN_STEBATES_AMDENG_EXPIRES='${GH_TOKEN_STEBATES_AMDENG_EXPIRES}'"
            [[ -n "${GH_TOKEN_SBATES130272_ROCM:-}" ]] && {
                echo ""
